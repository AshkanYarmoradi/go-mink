# projection-supervision Specification

## Purpose
Adds fault supervision to the projection engine: a configurable `RestartPolicy` that relaunches a `Faulted` async worker from its checkpoint with backoff, a manual `Restart` primitive symmetric with `Pause`/`Resume`/`Rebuild`, and a push-based state observer (`WithProjectionStateObserver`) plus an additive `ProjectionStateRestarting` state for observing self-healing recovery.
