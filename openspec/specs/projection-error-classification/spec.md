# projection-error-classification Specification

## Purpose
Lets the async projection worker distinguish transient failures from genuine poison events via an optional `AsyncOptions.ErrorClassifier`, so transient errors retry with backoff without consuming the poison retry budget or faulting the worker, and ships a `DefaultErrorClassifier` that recognizes standard transient signals.
