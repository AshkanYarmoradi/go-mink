...
- **GDPR erasure hardening** — closes the gaps a post-implementation audit found (all opt-in / optional, zero overhead when unused):
  - **Sibling-store erasure** — `SubjectErasable` + `DataEraser.WithSubjectStore`, with built-in `NewAuditSubjectEraser` / `NewSagaSubjectEraser` / `NewSnapshotSubjectEraser` (optional `adapters.SubjectAuditPurger` / `SubjectSagaPurger` on memory + postgres). Reaches PII derived from events — the plaintext audit trail, saga state, and snapshots — that crypto-shredding does not touch
  ...
  - **Subject index + backfill** — `SubjectIndexWriter` + `MemorySubjectIndex`, `WithSubjectIndexWriter` (append-time) / `WithResolverIndex`, and `BackfillSubjectIndex` — the migration step that makes pre-tagging historical subjects fully resolvable and erasable; turns discovery/erasure O(a subject's events)
